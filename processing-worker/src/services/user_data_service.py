...
class UserDataService:
    ...
    def __init__(self):
        ...
        if self.anon_key:
            logger.info(f"✅ [USER_DATA] Supabase anon key found (length: {len(self.anon_key)})")
        else:
            available_keys = [key for key in os.environ.keys() if 'SUPABASE' in key.upper()]
            logger.warning(f"❌ [USER_DATA] No Supabase anon key found. Available SUPABASE env vars: {available_keys}")
            logger.warning(f"❌ [USER_DATA] Config SUPABASE_ANON_KEY: {config.SUPABASE_ANON_KEY}")
    ...
    async def fetch_user_data(
        ...
        try:
            ...
            if not self.anon_key:
                logger.warning("[USER_DATA] Missing anon key, skipping user data fetch")
                return None
